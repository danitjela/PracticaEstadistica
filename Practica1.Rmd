...
El archivo Steam_2024_bestRevenue_1500.csv contiene una recopilación de datos de los 1500 juegos más rentables lanzados en Steam desde el 1 de enero de 2024, hasta el 9 de septiembre de 2024, utilizada para realizar un análisis descriptivo exhaustivo. Con este análisis nuestro equipo pretende realizar estimaciones sobre (COMPLETAR!!!).
...
En esta sección, presentamos un diccionario de datos que detalla cada variable contenida en nuestro conjunto de datos 'Steam_2024_bestRevenue_1500.csv'. Este diccionario es esencial para entender la naturaleza y el tipo de información que cada variable representa. Las variables abarcan desde COMPLETAR. Este entendimiento es crucial para el análisis posterior y la interpretación de los resultados.
...
Primero, generamos una tabla de frecuencias para la variable 'copiesSold', calculando la frecuencia absoluta y relativa de cada categoría. Esto nos ayuda a entender cuántas copias han sido vendidas de cada juego en toda nuestra muestra.

```{r}
# Calcula la tabla de frecuencias de la variable 'copiesSold' en el dataframe 'lab'.
table(lab$copiesSold) |> 
...
```{r}
# Calcula la tabla de frecuencias de la variable 'price' en el dataframe 'lab'.
table(lab$price) |> 
  # Convierte la tabla de frecuencias en un dataframe.
  as.data.frame() |> 
  # Añade una nueva columna 'F' calculando la frecuencia relativa acumulada.
