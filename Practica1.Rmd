...
Cabe destacar que nos han sorprendido los datos de una variable, la valoración. Ya que hay bastantes juegos que tienen un 0. Esto puede ser debido a un error en la carga del dataset en la página web.
Además, la variable steamId, no será utilizada porque no encontramos una utilidad para ella en nuestros objetivos.
...
En esta sección, presentamos un diccionario de datos que detalla cada variable contenida en nuestro conjunto de datos 'Steam_2024_bestRevenue_1500.csv'. Las variables abarcan desde COMPLETAR.
...
Primero, generamos un histograma para la variable 'avgPlaytime', utilizando el color 'steelblue' para las barras, lo que nos permite observar la distribución de las horas que las personas le dedican a los videojuegos.
...
lab |> ggplot(aes(avgPlaytime)) +
  ...
  theme_bw() +
    labs(x = "Tiempo promedio de juego (horas)", y = "Frecuencia", 
         title = "Distribución de tiempo de juego por intervalos")
