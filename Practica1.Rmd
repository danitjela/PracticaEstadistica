...
Además, vamos a descartar el estudio la variable "steamId", ya que no nos aporta ningún tipo de información relevante para nuestro objetivo.
...
Para el estudio de la variable "releaseDate" crearemos un histograma, donde se agruparán las fechas de salida de cada videojuego en meses (desde enero hasta septiembre de este mismo año), y determinaremos cuál han sido los que tienen más y menos lanzamientos.
...
A continuación, nos enfocamos en el estudio del tiempo que dedican las personas a cada uno de los juegos. Una herramienta crucial para entender la distribución de nuestra variable cuantitativa es el histograma, ya que, este gráfico nos permitirá observar de manera sencilla la forma y la dispersión de los datos.
...
También hay que destacar que hemos impuesto un límite de 100 en las horas de juego, porque solo unos pocos videojuegos superaban esta cifra, y en el histograma no se observaba de forma correcta. Para ello, hemos creado una tabla a continuación.
...
En esta tabla observamos los 10 juegos a los que las personas les han dedicado más horas, para de esta manera, completar el estudio de esta variable.
...
Esta variable la estudiaremos para poder ver más adelante si se puede relacionar con la recaudación de los juegos. A continuación, estudiaremos las frecuencias relativas y absolutas por rangos, esto se debe a la cantida de valores que hay.
...
Como se puede observar, hay bastantes valores de 0 que, como hemos comentado anteriormente, debe tratarse de un error. Por ello, lo hemos sacado fuera de los rangos.

Para continuar con el estudio, eliminaremos ese valor para poder hacerlo de una manera más fiel a la realidad. Para esto, utilizaremos el siguiente código.
...
A continuación, realizaremos un histograma.
...
Además de este histograma, hemos realizado un resumen númerico a partir del cual, podremos comprender los gráficos con facilidad.
...
Observamos como las puntuaciones, por lo general, son extremadamente altas. Esto se puede deber a que el dataset es sobre los 1500 mejores juegos según su recaudación, por lo que podemos entender que son buenos juegos. Además, hay que tener en cuenta que estas valoraciones son puestas por los usuarios y no por críticos profesionales.

### Estudio de la variable PUBLISHERCLASS

Ahora, analizaremos la variable "publisherclass". En esta variable, veremos las diferencias entre los tipos de empresas que hay y como se distribuyen. A continuación, gracias al siguiente código, estudiaremos las frecuencias relativas y absolutas.
...
Para observar estos datos con mayor claridad, utilizaremos una gráfica en función de la frecuencia relativa.
...
Como podemos observar, la mayor parte de los valores obtenidos, son del tipo "Indie", que es como se denota la empresa pequeña, ocupando el 87% de los casos. Además, se ve un caso aislado, "Hobbyist", que es como se denota a un particular que hace el juego por afición.
...
Ahora, vamos a analizar la variable "developers". Realmente para nuestro estudio no es de gran importancia, aun así, crearemos una tabla de frecuencia para saber cuáles son los 10 desarrolladores más frecuentes, ya que, puede ser curioso saberlo.
...
En la tabla podemos observar claramente los 10 desarrolladores más frecuentes entre los 1407 desarrolladores únicos. Como ya hemos mencionado, esta variable no será de gran importancia a la hora de responder a nuestras preguntas y de cumplir el objetivo final de la práctica.

## Datos bivariantes

En este apartado, analizaremos las relaciones entre las variables para poder responder a las preguntas planteadas en la introducción y cumplir con el objetivo principal de nuestro estudio.
...
El gráfico muestra una concentración de juegos con un tiempo promedio entre 0 y 100 horas, y otra gran concentración en los valores de recaudación. Sin embargo, a pesar de que algunos juegos tienen tiempos de juego elevados (cercanos a las 300 horas), estos no necesariamente generan más ingresos que otros con menos tiempo jugado. Por no decir que, el juego con mayor recaudación se encuentra entre 0 y 50 horas.
...
A modo de conclusión podemos decir que, el tiempo de juego tiene una influencia mínima o casi inexistente en la recaudación.
...
Podemos apreciar cómo la mayoría de puntos se agrupan en la parte baja de la gráfica indicando que pese a la puntuación alta que puedan tener, son pocos los que tienen recaudaciones altas. Lo que sí queda claro es que de los juegos con poca puntuación, ninguno sobresale. De todas formas, comprobemos esto mediante un análisis de regresión para obtener un modelo cuantitativo.
...
ggplot(lab, aes(x = publisherClass, y = revenue)) +
  ...
  labs(title = "Recaudación por tipo de empresa",
       x = "Tipo de empresa",
       y = "Recaudación promedio (en millones)") +
  scale_y_continuous(labels = label_number(scale = 1e-6, suffix = "M")) +
  theme_minimal()
